...
IndexRTrials_Pearson = size(Table,2)+1;
IndexRTrials_Spearman = size(Table,2)+2;
IndexRStim_Pearson = size(Table,2)+3;
IndexRStim_Spearman = size(Table,2)+4;
...
if BlockOfInterest == 1
    tmpStartEndStimuli = [2 5];
elseif BlockOfInterest == 2
    tmpStartEndStimuli = [10 13];
elseif BlockOfInterest == 3
    tmpStartEndStimuli = [18 21];
else
    'Error'
    dasdasda
end
    ...
    try
        ...
        for counterSweep = 1:size(SweepsPaths,2)
            ...
            if strcmp(currSweepName, SelectedSweepName)
                ...
                WindowOfInterest = tmpStartEndStimuli*sRate;
                ...
                SelectedSweepStim{end+1} = currSweepStim(WindowOfInterest(1):WindowOfInterest(2)); 
        ...
        if size(SelectedSweeps,2)>1
            ...
            %plot((1:size(currSweepData,1))./sRate,currSweepData)
            
            
            %IndexRStim IndexRTrials
            RStim_Pearson = [];
            RStim_Spearman = [];
            for counterStim = 1:size(SelectedSweepData,2)
                tmp = corr(SelectedSweepData{counterStim},SelectedSweepStim{counterStim},'Type','Pearson');
                RStim_Pearson(counterStim) = tmp;
                tmp = corr(SelectedSweepData{counterStim},SelectedSweepStim{counterStim},'Type','Spearman');
                RStim_Spearman(counterStim) = tmp;
            end
            
            RTrials_Pearson = [];
            RTrials_Spearman = [];
            for counterTrials1 = 1:size(SelectedSweepData,2)
                for counterTrials2 = (counterTrials1+1):size(SelectedSweepData,2)
                tmp = corr(SelectedSweepData{counterTrials1},SelectedSweepData{counterTrials2},'Type','Pearson');
                RTrials_Pearson(end+1) = tmp;
                tmp = corr(SelectedSweepData{counterTrials1},SelectedSweepData{counterTrials2},'Type','Spearman');
                RTrials_Spearman(end+1) = tmp;
                end
            end
            ...
            if FirstCycle
                FirstCycle = 0;
                Table(:,end+1:end+4)=array2table(nan([size(Table,1) 4]));
                Table.Properties.VariableNames(IndexRTrials_Pearson) = {'RTrials_Pearson'};
                Table.Properties.VariableNames(IndexRTrials_Spearman) = {'RTrials_Spearman'};
                Table.Properties.VariableNames(IndexRStim_Pearson) = {'RStim_Pearson'};
                Table.Properties.VariableNames(IndexRStim_Spearman) = {'RStim_Spearman'};
                
                
                Table.RTrials_Pearson(currIndexTable)=nanmean(RTrials_Pearson);
                Table.RTrials_Spearman(currIndexTable)=nanmean(RTrials_Spearman);
                Table.RStim_Pearson(currIndexTable)=nanmean(RStim_Pearson);
                Table.RStim_Spearman(currIndexTable)=nanmean(RStim_Spearman);
...
save(['TableSubthresholdCorr_' SelectedSweepName '_' num2str(BlockOfInterest)],'Table')
